...
// ParseLogLevel parses a string into a LogLevel. Matching is case-insensitive
// and accepts the common aliases that apps, Alloy, and Loki emit (e.g. "error",
// "warning", "fatal"). Returns an error for unrecognized level names.
...
type LogEntry struct {
	...
	IngestedAt time.Time // arrival timestamp (set by logd on ingestion)
	Level      LogLevel  // severity: DEBUG=0, INFO=1, WARN=2, ERROR=3
	ServiceID  uint16    // lookup table ID for the originating service
	Message    string    // log message body
	Extra      string    // raw JSON containing trace_id, span_id, and other fields
}
