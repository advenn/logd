...
func LoadManifest(dirPath string) (*Manifest, error) {
	...
	for _, e := range entries {
		manifest.segments[e.ID] = e
		manifest.byMinTS = append(manifest.byMinTS, e)
		if e.ID >= manifest.nextID {
			manifest.nextID = e.ID + 1
		}
	}

	sort.Slice(manifest.byMinTS, func(i, j int) bool {
		return manifest.byMinTS[i].MinTS < manifest.byMinTS[j].MinTS
	})
	return manifest, nil
}
...
func (m *Manifest) Filter(minTS, maxTS int64) []*SegmentMeta {
	...
	for _, meta := range m.byMinTS {
		if meta.MaxTS >= minTS && meta.MinTS <= maxTS {
