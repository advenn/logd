...
// Size returns the number of interned entries. The writer uses it to detect when
// new services have appeared since the table was last persisted, so it can flush
// lookup.bin durably before any entry stamped with a new ID becomes durable.
func (lt *LookupTable) Size() int {
	lt.mu.RLock()
	defer lt.mu.RUnlock()
	return len(lt.idToName)
}
